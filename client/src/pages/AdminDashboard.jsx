  ...
  return (
    <div style={{ padding: '30px', maxWidth: '1200px', margin: '0 auto',
      ...
      <div style={{ background: 'linear-gradient(135deg, #166534, #15803d)', color: 'white',
        padding: '25px', borderRadius: '12px', display: 'flex',
        ...
        <div>
          ...
          <p style={{ margin: '5px 0 0', opacity: 0.85, fontSize: '13px' }}>
            Inventory, orders, analytics and fulfillment controls
          </p>
        </div>
          ...
          {[
            ...
            <div key={stat.label} style={{ backgroundColor: 'rgba(255,255,255,0.15)',
              ...
              <div style={{ fontSize: '10px', opacity: 0.8, textTransform: 'uppercase',
                letterSpacing: '1px' }}>{stat.label}</div>
              <div style={{ fontSize: '18px', fontWeight: 'bold', marginTop: '2px' }}>{stat.value}</div>
            </div>
          ))}
          {lowStock.length > 0 && (
            <div style={{ backgroundColor: '#dc2626', padding: '12px 18px',
              borderRadius: '8px', textAlign: 'center', border: '1px solid #f87171' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '25px' }}>
        {[
          ...
          <button key={tab.key} onClick={() => setActiveTab(tab.key)}
            style={{ padding: '11px 22px', borderRadius: '8px', fontWeight: 'bold',
              cursor: 'pointer', fontSize: '14px', transition: 'all 0.2s',
              backgroundColor: activeTab === tab.key ? '#166534' : 'white',
              color: activeTab === tab.key ? 'white' : '#334155',
              border: activeTab === tab.key ? 'none' : '1px solid #cbd5e1',
              boxShadow: activeTab === tab.key ? '0 2px 8px rgba(22,101,52,0.3)' : 'none' }}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      {error && (
        <div style={{ backgroundColor: '#fee2e2', color: '#991b1b', padding: '12px',
          borderRadius: '8px', marginBottom: '15px' }}>{error}</div>
      )}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            <thead>
              <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                {['ID / Customer', '📍 Location', 'Items', 'Total', 'Status'].map(h => (
                ...
                <tr key={order._id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <td style={{ padding: '14px 16px' }}>
                    <select value={order.status}
                      ...
                      style={{ padding: '7px 10px', borderRadius: '6px', border: '1px solid #e2e8f0',
                        fontWeight: 'bold', cursor: 'pointer', fontSize: '13px',
                        backgroundColor: order.status === 'Pending' ? '#fef3c7'
                          : order.status === 'Delivered' ? '#dcfce7'
                          : order.status === 'Paid' ? '#dbeafe' : '#f1f5f9',
                        color: order.status === 'Pending' ? '#b45309'
                          : order.status === 'Delivered' ? '#15803d'
                          : order.status === 'Paid' ? '#1d4ed8' : '#334155' }}>
      ...
      {activeTab === 'inventory' && (
        <>
          ...
          {lowStock.length > 0 && (
            <div style={{ backgroundColor: '#fef3c7', border: '1px solid #f59e0b',
              ...
              <span style={{ color: '#92400e' }}>
                {lowStock.map(p => `${p.name} (${p.countInStock} left)`).join(' · ')}
              </span>
            </div>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between',
            ...
            <button onClick={() => { resetForm(); setShowAddForm(!showAddForm); }}
              style={{ padding: '10px 20px', fontWeight: 'bold', cursor: 'pointer',
                ...
                backgroundColor: showAddForm ? '#dc2626' : '#15803d', color: 'white' }}>
          ...
          {/* ── Product Form ── */}
          {showAddForm && (
            <div style={{ backgroundColor: '#f8fafc', border: '1px solid #e2e8f0',
              borderRadius: '12px', padding: '25px', marginBottom: '25px' }}>
              ...
              <form onSubmit={handleProductSubmit}>
                <div style={{ display: 'grid',
                  ...
                  <div>
                    ...
                    <input required value={formData.name} style={inp}
                      placeholder="e.g., Mumias Sugar 2kg"
                      onChange={e => setFormData({...formData, name: e.target.value})} />
                  </div>

                  <div>
                    <label style={lbl}>Category *</label>
                    <select required value={formData.category} style={inp}
                  ...
                  <div>
                    ...
                    <input value={formData.brand} style={inp} placeholder="e.g., Mumias"
                      onChange={e => setFormData({...formData, brand: e.target.value})} />
                  </div>

                  <div>
                    <label style={lbl}>Retail Price (KSh) *</label>
                    <input required type="number" min="0" value={formData.retailPrice}
                      style={inp} placeholder="e.g., 230"
                      onChange={e => setFormData({...formData, retailPrice: e.target.value})} />
                  </div>

                  <div>
                    <label style={lbl}>Wholesale Price (KSh) *</label>
                    <input required type="number" min="0" value={formData.wholesalePrice}
                      style={inp} placeholder="e.g., 210"
                      onChange={e => setFormData({...formData, wholesalePrice: e.target.value})} />
                  </div>

                  <div>
                    <label style={lbl}>Stock Quantity *</label>
                    <input required type="number" min="0" value={formData.countInStock}
                      style={inp} placeholder="e.g., 50"
                  ...
                  <div style={{ gridColumn: '1 / -1' }}>
                    ...
                    <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start',
                      ...
                      <div onClick={() => fileInputRef.current.click()}
                        ...
                        {imagePreview ? (
                          ...
                          <div style={{ textAlign: 'center', color: '#94a3b8' }}>
                            ...
                            <div style={{ fontSize: '11px', marginTop: '5px' }}>Click to upload</div>
                          </div>
                        )}
                      </div>
                      ...
                      <div style={{ flex: 1, minWidth: '200px' }}>
                        ...
                        <input value={formData.image} style={{...inp, fontSize: '12px'}}
                  ...
                  <div style={{ gridColumn: '1 / -1' }}>
                    ...
                    <textarea value={formData.description} rows="2" style={{...inp, resize: 'vertical'}}
                ...
                <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
                  <button type="submit" disabled={submitting || uploadingImage}
                    style={{ padding: '12px 30px', backgroundColor: submitting ? '#86efac' : '#15803d',
                      color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold',
                      cursor: submitting ? 'not-allowed' : 'pointer', fontSize: '15px' }}>
                    {uploadingImage ? '📤 Uploading image...'
                      : submitting ? '💾 Saving...'
                      : editingProduct ? '💾 Save Changes' : '➕ Add Product'}
                  </button>
                  <button type="button" onClick={resetForm}
                    style={{ padding: '12px 20px', backgroundColor: 'white', color: '#64748b',
              ...
              <thead>
                <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                  {['Product', 'Category', 'Retail', 'Wholesale', 'Stock', 'Actions'].map(h => (
                    <th key={h} style={{ padding: '13px 15px', textAlign: 'left',
                      color: '#475569', fontWeight: '600' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {products.length === 0 ? (
                  ...
                  <tr key={p._id} style={{ borderBottom: '1px solid #f1f5f9',
                    backgroundColor: p.countInStock <= 5 ? '#fffbeb' : 'white',
                    ...
                    <td style={{ padding: '12px 15px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        {p.image ? (
                          ...
                          <div style={{ width: '48px', height: '48px', backgroundColor: '#f1f5f9',
                    ...
                    <td style={{ padding: '12px 15px' }}>
                      <span style={{ backgroundColor: '#dcfce7', color: '#15803d',
                        padding: '3px 10px', borderRadius: '999px',
                        fontSize: '12px', fontWeight: '600' }}>{p.category}</span>
                    </td>
                    <td style={{ padding: '12px 15px', fontWeight: 'bold', color: '#15803d' }}>
                    ...
                    <td style={{ padding: '12px 15px' }}>
                      <span style={{ padding: '4px 10px', borderRadius: '999px',
                        fontSize: '12px', fontWeight: 'bold',
                        backgroundColor: p.countInStock <= 5 ? '#fee2e2'
                          : p.countInStock <= 15 ? '#fef3c7' : '#dcfce7',
                        color: p.countInStock <= 5 ? '#dc2626'
                          : p.countInStock <= 15 ? '#b45309' : '#15803d' }}>
                    ...
                    <td style={{ padding: '12px 15px' }}>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        ...
                        <button onClick={() => handleDelete(p._id, p.name)}
                          style={{ padding: '6px 14px', backgroundColor: '#dc2626',
                            color: 'white', border: 'none', borderRadius: '6px',
                            cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>
                          🗑️ Delete
                        </button>
